...
export const EditPresenter: FC<Props> = (props) => {
  ...
  const beforeDifficulty = targetQuest?.difficulty as Difficulty;
  const beforeDates = targetQuest?.dates.map((v) => fromWeekToNumber(v)) ?? [];

  const [difficulty, setDifficulty] = useState<Difficulty>(beforeDifficulty);
  const [dates, setDates] = useState<number[]>(beforeDates);
          ...
          <div className="my-2 col-span-5">
            ...
            <div className="grid grid-cols-5 my-2">
              ...
              <div className="col-span-3 flex justify-end">
                <input
                  type="number"
                  className="w-10 border-2 rounded p-1 mr-2"
